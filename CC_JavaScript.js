...
while (true) {
    ...
    //--Make array for recipes (empty every round)
    let recipeIds = []
    ...
    for (let i = 0; i < actionCount; i++) {
        ...
        //-- Store recipe IDs in array
        recipeIds.push(actionId);
    }
    for (let i = 0; i < 2; i++) {
        var inputs = readline().split(' ');
    ...
    //--Try to brew random recipe (not good way :D)
    console.log('BREW ' +  recipeIds[Math.floor(Math.random() * recipeIds.length)]);
